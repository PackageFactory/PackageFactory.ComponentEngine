...
trait ResolveTypeReference
{
    ...
    private function resolveTypeReference(
        ...
        $type = $this->scope->lookupType($typeReferenceNode->name)
            ?? throw new \Exception('@TODO: Unknown Type ' . $typeReferenceNode->name . ' ' . $this::class);

        return $typeReferenceNode->isArray
            ? ArrayType::of($type)
            : $type;
    }
}
